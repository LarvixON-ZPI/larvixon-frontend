# Terms of Service

**Effective Date:** TBD
**Last Updated:** TBD

---

## 1. Acceptance of Terms

By accessing or using Larvixon ("Service", "Platform"), you agree to be bound by these Terms of Service ("Terms"). If you do not agree to these Terms, you must not use the Service.

**About this project:**
- **Project:** LarvixON - AI-powered toxin detection platform
- **Developer:** TBD
- **Institution:** Wroclaw University of Technology
- **Purpose:** Academic research and demonstration (non-commercial)
- **Status:** Experimental research prototype

---

## 2. Service Description

Larvixon is an **experimental research platform** developed as part of an academic project at Poznan University of Technology. The platform uses AI to analyze larvae behavior in video recordings for toxin detection purposes.

### 2.1 Features

The Service includes:
- Upload and storage of video files showing larvae exposed to patient plasma samples
- AI-based automated analysis of larval movement patterns
- Detection and identification of toxic substances with confidence scores
- Generation of analysis reports
- Interactive 3D simulation of larvae behavior (Unity WebGL)
- User account management and analysis history

### 2.2 Academic/Research Use Only

This platform is intended for:
- ✅ Academic research and educational purposes
- ✅ Laboratory testing and experimentation
- ✅ Demonstration of AI/ML capabilities
- ✅ Proof-of-concept validation

### 2.3 NOT Intended For

- ❌ **Clinical diagnosis or medical treatment** decisions
- ❌ **Commercial or production use** without proper validation
- ❌ **Emergency medical situations** requiring immediate decisions
- ❌ **Processing real patient data** without appropriate ethical approval

**IMPORTANT:** This is a research prototype. Results should not be used for actual medical decisions without further validation and regulatory approval.

---

## 3. User Accounts and Responsibilities

### 3.1 Registration Requirements

To use the Service, you must:
- Provide a valid email address
- Be at least **18 years of age**
- Use the platform for legitimate research or educational purposes
- Comply with your institution's research ethics guidelines

### 3.2 Account Security

You are responsible for:
- ✅ Maintaining the confidentiality of your password
- ✅ All activities that occur under your account
- ✅ Notifying us immediately of any unauthorized access
- ✅ Using a strong password (minimum 8 characters)

### 3.3 Prohibited Activities

You agree NOT to:
- ❌ Use automated tools, bots, or scrapers to overwhelm the service
- ❌ Upload files containing malicious code, viruses, or malware
- ❌ Attempt to reverse-engineer or extract the AI models
- ❌ Upload videos containing identifiable patient data (faces, names, medical record numbers)
- ❌ Use the Service for any illegal or harmful purposes
- ❌ Attempt to gain unauthorized access to the system
- ❌ Share your account credentials with others

**Consequences:** Violation may result in account suspension or termination.

---

## 4. Acceptable Use Policy

### 4.1 Uploaded Content

By uploading video files, you confirm that:
- You have the legal right to upload all submitted content
- Videos do not violate patient privacy laws (GDPR or equivalent)
- You have obtained necessary informed consents (if using real patient samples)
- Content does not infringe on third-party rights

### 4.2 Video Requirements

**Accepted formats:** MP4, AVI, MOV  
**Maximum file size:** 3 GB per video  
**Resolution:** Minimum 720p recommended  
**Content:** Must depict larvae behavior under controlled conditions

### 4.3 Prohibited Content

Do NOT upload videos containing:
- ❌ Identifiable patient information (faces, names, birthdates, ID numbers)
- ❌ Inappropriate, illegal, or harmful content
- ❌ Spam or irrelevant test files
- ❌ Protected health information (PHI) without proper de-identification

---

## 5. Intellectual Property Rights

### 5.1 Project Ownership

This platform was developed as an **academic project** at Poznan University of Technology.

**Intellectual property rights:**
- Platform source code: TBD
- AI models and algorithms: Owned by the project team
- Documentation and materials: TBD

### 5.2 Your Content

**You retain ownership** of all video files you upload.

**License you grant to us:** By uploading content, you grant us a limited license to:
- Store and process your videos for analysis purposes
- Display results back to you
- Use anonymized data to improve the AI model (only with your consent)

**Data retention:** You can delete your videos at any time. We will remove your data within 30 days of deletion.

### 5.3 AI-Generated Results

Analysis results are provided "as-is" for research purposes:
- **You may:** Use results for research, education, and academic publications
- **Citation required:** If publishing research using this platform, please cite our project appropriately

---

## 6. Research Disclaimer

### 6.1 Experimental Nature

⚠️ **IMPORTANT DISCLAIMER:**

Larvixon is an **experimental research prototype** developed for academic purposes. It is **NOT**:
- A medical device approved for clinical use
- Validated for diagnostic purposes
- Suitable for making medical decisions
- Guaranteed to be accurate or reliable in all cases

**Results are for research and educational purposes only.**

### 6.2 No Medical Advice

This Service does **NOT** provide:
- Medical advice, diagnosis, or treatment recommendations
- Clinical decision support validated for patient care
- Results suitable for direct medical use without further validation

**You acknowledge that:**
- This is a proof-of-concept research tool
- Results have not been clinically validated
- AI predictions may contain errors or inaccuracies
- Professional medical judgment is always required

### 6.3 No Warranty

We make **NO WARRANTY** that:
- Analysis results will be accurate or complete
- The Service will detect all toxic substances
- Results will be suitable for any specific purpose

---

## 7. Service Availability

### 7.1 Best Effort Service

As an academic project with limited resources, we provide the Service on a **"best effort" basis**:
- ⚠️ Service may be **unavailable** due to maintenance, server issues, or resource limitations
- ⚠️ We do **NOT guarantee** specific uptime or response times
- ⚠️ Service may be **discontinued** when the academic project ends

### 7.2 No Service Level Agreement (SLA)

There is **no SLA** for this research platform. We will do our best to:
- Keep the service running during the project period
- Notify users of planned maintenance
- Fix critical bugs when possible

---

## 8. Data Privacy (GDPR Compliance)

### 8.1 Data Controller

For personal data processing purposes:
- **Data Controller:** TBD
- **Purpose:** Academic research project
- **Legal basis:** Consent (GDPR Art. 6(1)(a)) and legitimate interest for research (Art. 6(1)(f))

### 8.2 Data We Collect

- **Account data:** Email, name (optional), password (hashed)
- **Video files:** Uploaded by you for analysis
- **Usage data:** Login times, analysis history, IP addresses (for security)

### 8.3 Data Retention

- **Video files:** Stored while your account is active, deleted within 30 days after account deletion
- **Account data:** Deleted within 30 days of account deletion request
- **Anonymized research data:** May be retained indefinitely for academic purposes (if you consent)

### 8.4 Your Rights

Under GDPR, you have the right to:
- Access your personal data
- Correct inaccurate data
- Delete your account and data ("right to be forgotten")
- Export your data in machine-readable format
- Object to data processing
- Lodge a complaint with your data protection authority (Poland: UODO - uodo.gov.pl)

**Contact:** TBD

---

## 9. Limitation of Liability

### 9.1 No Liability

**TO THE MAXIMUM EXTENT PERMITTED BY LAW:**

This is a **free academic research project** provided "as-is" without any warranties.

We (the project team and Poznan University of Technology) are **NOT LIABLE** for:
- ❌ Any damages arising from use of the Service
- ❌ Loss of data, time, or research results
- ❌ Inaccurate or misleading analysis results
- ❌ Service downtime or interruptions
- ❌ Medical decisions made based on this experimental tool

**USE AT YOUR OWN RISK.**

### 9.2 No Commercial Relationship

This is a **non-commercial academic project**:
- ✅ **Free to use** for research and educational purposes
- ✅ **No paid features or subscriptions**
- ✅ **No commercial support** (community/academic support only)

---

## 10. Termination

### 10.1 Account Termination

You may delete your account at any time via **Settings > Delete Account** or by emailing us.

We may suspend or terminate accounts that:
- Violate these Terms
- Upload prohibited content
- Abuse the service (excessive uploads, malicious activity)

### 10.2 Project Discontinuation

This research project may be discontinued at any time:
- ⚠️ We will provide **at least 30 days' notice** before shutting down
- ⚠️ You will be able to download your data before service closure
- ⚠️ After closure, all user data will be deleted (except anonymized research data)

---

## 11. Governing Law

These Terms are governed by the laws of **Poland**.

For disputes:
- **Informal resolution:** Contact us first at TBD
- **Jurisdiction:** Courts of Wroclaw, Poland

---

## 12. Changes to These Terms

We may update these Terms to reflect changes in the project scope or legal requirements.

**Notification:** We will notify users via email at least 14 days before significant changes take effect.

---

## 13. Contact Information

### Project Team
**Email:** TBD  
**Institution:** Wroclaw University of Technology  
**Department:** TBD

### Data Privacy
**Email:** TBD

### Technical Issues
**Email:** TBD

---

## 14. Academic Citation

TBD

## ⚠️ Research Ethics Notice

If you plan to use this platform with real patient samples, ensure you have:
- ✅ Institutional Review Board (IRB) or Ethics Committee approval
- ✅ Informed consent from patients
- ✅ Compliance with your institution's research data policies
- ✅ Proper de-identification of patient data

**This platform does not provide ethics approval or compliance certification.**
